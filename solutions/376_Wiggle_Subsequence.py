...
class Solution(object):
    def wiggleMaxLength(self, nums):
        ...
        for cur_idx,cur_num in enumerate(nums):
            for pre_idx,pre_num in enumerate(nums[:cur_idx]):
                if cur_num > pre_num:
                    more[cur_idx] = max(more[cur_idx],less[pre_idx]+1)
                if cur_num < pre_num:
                    less[cur_idx] = max(less[cur_idx],more[pre_idx]+1)
        return max(max(more),max(less))


if __name__ == '__main__':
    nums_1 = [1,7,4,9,2,5]
    nums_2 = [1,17,5,10,13,15,10,5,16,8]
    nums_3 = [1,2,3,4,5,6,7,8,9]
    print Solution().wiggleMaxLength(nums_1)
    print Solution().wiggleMaxLength(nums_2)
    print Solution().wiggleMaxLength(nums_3)
