...
class Solution(object):
                ...
                for i in xrange(len(key)):
                    ...
                    for j in 'abcdefghijklmnopqrstuvwxyz':
                        if key[i]!=j:
                            ...
                            if next_word in wordList and next_word not in begin_dict:
                                cur_dict[next_word] = value + 1
                                wordList.remove(next_word)
                                exit_flag = True
                # if self.is_next_word_i(key,wordList) and wordList[i] not in begin_dict:
                #     cur_dict[wordList[i]] = value + 1
                #     wordList.remove(wordList[i])
                #     exit_flag = True
            begin_dict = cur_dict
        return begin_dict[endWord] if endWord in begin_dict else 0

    def ladderLength_LTE_ii(self, beginWord, endWord, wordList):
        """
        :type beginWord: str
        :type endWord: str
        :type wordList: List[str]
        ...
        while exit_flag:
            ...
            for key, value in begin_dict.items():
                if key == endWord:
                    return value
                for i in xrange(len(wordList)-1,-1,-1):
                    if self.is_next_word(key,wordList[i]) and wordList[i] not in begin_dict:
                        cur_dict[wordList[i]] = value + 1
                        wordList.remove(wordList[i])
                        exit_flag = True
            begin_dict = cur_dict
    ...
    def ladderLength_LTE(self, beginWord, endWord, wordList):
        """
        :type beginWord: str
        :type endWord: str
        :type wordList: List[str]
