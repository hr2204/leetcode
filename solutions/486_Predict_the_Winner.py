...
class Solution(object):
    ...
    def PredictTheWinner_LTE(self, nums):
        """
        :type nums: List[int]
        :rtype: bool
        """
    ...
    def getMax(self,nums,start,end):
        if start == end:
            ...
            return max(nums[start]-self.getMax(nums,start+1,end),nums[end]-self.getMax(nums,start,end-1))

if __name__ == '__main__':
    input = [1, 5, 233, 7]
    print Solution().PredictTheWinner(input)